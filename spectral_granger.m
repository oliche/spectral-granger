function [M,S] = spectral_granger(X,fs,fRes)
%Usage: [M] = spectral_granger(X,fs,fRes)
...
%
function X = vectorized_solve(A, B, iter)
% vectorized solving of a 2x2 system of equations
complex_conjugate = true;
if nargin <= 2, iter = true; end
X = zeros(2, 2, size(A,3));
a = A(1, 1, :);
b = A(1, 2, :);
d = A(2, 1, :);
e = A(2, 2, :);
for m = 1:2  % loop over columns
...
% one more round to restore machine precision
if ~iter, return, end
dx = vectorized_matmul22(A, X) - conj(permute(B, [2, 1, 3]));
X = X - vectorized_solve(A, dx, false);
end

function g = vectorized_matmul22_conj(a, b)
g = vectorized_matmul22(a, conj(permute(b, [2, 1, 3])));
end

function g = vectorized_matmul22(a, b)
g = zeros(2, 2, size(a, 3));
...
function [H, Z, ps, ps0, converged, relerr] = wilson_sf(S, fs, tol, perf)
if (nargin < 3) || isempty(tol), tol = 1e-9; end
if (nargin < 4) , perf=true; end
...
while ( (niter < maxiter) && ~converged )
    ...
    % performance switch for cases where k=1 or k=2
    switch k .* perf
        case 1
            g = abs((U ./ ps) .^2) + I;
        case 2
            % do a vectorized 2x2 matrix operation to match as below
%             detps = ps(1, 1, :) .* ps(2, 2, :) - ps(1, 2, :) .* ps(2, 1, :);
%             ips = 1./ detps .* [ps(2, 2, :),  - ps(1, 2, :) ; - ps(2, 1, :) , ps(1, 1, :)];
%             gsav = vectorized_matmul22_conj(vectorized_matmul22(ips, Sarr), ips) + I;
            v = vectorized_solve(ps, U, false);
            g = vectorized_matmul22_conj(v, v) + I; 
    ...
    switch k .* perf
        case 1
            ps = ps .* gp + T;
        case 2
            ps = vectorized_matmul22(ps, gp + T);
        otherwise
            for i = 1 : M
                ps(:,:,i) = ps(:,:,i)*(gp(:,:,i) + T);
            end
